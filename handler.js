          ...
          for (let user of participants) {
           ...
           // let pp = 'https://i.ibb.co/jr9Nh6Q/Thumb.jpg'
            try {
              ...
              let wel = `https://hardianto-chan.herokuapp.com/api/tools/welcomer2?name=${encodeURIComponent(this.getName(user))}&descriminator=${user.split(`@`)[0].substr(-5)}&totalmem=${encodeURIComponent(groupMetadata.participants.length)}&namegb=${encodeURIComponent(this.getName(jid))}&ppuser=${pp}&background=https://i.ibb.co/7CM77tx/c180e14f43cf.jpg&apikey=hardianto`
...
`.trim(), '', 'DISABLE DELETE', '#off antidelete', {
    ...
    async onCall(json) {
        ...
        await this.sendMessage(from, 'Bot Tidak Menerima Panggilan. Maaf Anda akan diblokir, silakan hubungi pemilik bot ini untuk membuka blokir ini wa.me/48729759504', MessageType.extendedText)
...
global.dfail = (type, m, conn) => {
  let msg = {
    ...
    owner: '*「❗」OWNER ONLY*',
    mods: '*「❗」MODERATOR ONLY*',
    premium: 'FITUR INI KHUSUS UNTUK PREMIUM KETIK #owner UNTUK MEMBELI PREMIUM',
    group: '*「❗」GROUP ONLY*',
    private: '*「❗」PRIVATE ONLY*',
    admin: '*「❗」ADMIN GROUP ONLY*',
    botAdmin: '*「❗」 BOT ADMIN ONLY*',
    unreg: '*「 NOT REGISTERED 」*\n\n*Kamu Belum Terdaftar Dalam Database YukiBot*\n\nAda 2 Cara Untuk Register :\n1. Ketik #registered\n2. Ketik #daftar nama.umur\nContoh Daftar Nya #daftar YukiBot.15',
